...
Bigbytes will look for a file named `profiles.yml` located in your dbt project root
folder.

For example, if your Bigbytes project is named `demo_project` and your dbt project
...
Using Bigbytes, you may also [interpolate variables](/dbt/variable-interpolation) in your connection profiles.
...
target that Bigbytes uses when running a dbt model block.

The command Bigbytes executes is `dbt run` with the flag `--target` and the value of
