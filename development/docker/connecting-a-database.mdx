...
icon: "database"
description: "Here's how you can connect a local database to Bigbytes in Docker for development."
---

## Overview

When you develop in Docker, you can connect a database to your Bigbytes container. This allows you to use the database in your data pipelines.

A Docker Network is a virtual network that allows containers to communicate with each other.
By creating a network, you can connect database containers to your Bigbytes instances.
Read more about Docker Networks [here](https://docs.docker.com/network/).

The following guides detail various methods for creating databases and connecting them to your Bigbytes project.
...
If you're using Bigbytes in docker compose, you can easily connect a PostgreSQL database to your Bigbytes container. To do so, first define a `.env` file with the following variables:
...
The following steps will create a PostgreSQL database container and connect it to a Bigbytes container.

<Steps>
<Step title="Create Docker network">
  ```bash
   docker network create bigbytes-app
   ```
</Step>
<Step title="Start PostgreSQL Docker container">
   ```bash
   docker run --network bigbytes-app --network-alias postgres_db \
...
<Step title="Launch Bigbytes with `BIGBYTES_DATABASE_CONNECTION_URL`">
```bash
   docker run --network bigbytes-app -it -p 6789:6789 -v $(pwd):/home/src \
      -e BIGBYTES_DATABASE_CONNECTION_URL=postgresql+psycopg2://<username>:<password>@postgres_db:5432/<database> \
   ...
   ```bash
   docker run --network bigbytes-app -it -p 6789:6789 -v $(pwd):/home/src \
      -e BIGBYTES_DATABASE_CONNECTION_URL="postgresql+psycopg2://<username>:<password>@postgres_db:5432/<database>?options=-c%%20search_path%%3D<schema_name>" \
      getbigbytes/bigbytes \
      /app/run_app.sh bigbytes start [project_name]
   ```
</Step>
...
<Step title="Start MSSQL Docker container">
   ```bash
   docker run --network bigbytes-app --network-alias mssql_db \
...
```bash
   docker run --network bigbytes-app -it -p 6789:6789 -v $(pwd):/home/src \
   -e BIGBYTES_DATABASE_CONNECTION_URL="mssql+pyodbc://?odbc_connect=DRIVER={ODBC Driver 18 for SQL Server};SERVER=mssql_db;DATABASE=master;UID=SA;PWD=test_Password123;ENCRYPT=yes;TrustServerCertificate=yes;" \
   getbigbytes/bigbytes \
   /app/run_app.sh bigbytes start [project_name]
   ```
