...
| `LDAP_ROLES_MAPPING` | Mapping of user LDAP group to Bigbytes roles. Must be in JSON string format. | `'{"Admin": ["Admin"]}'` |
...
When LDAP authentication is enabled, users will need to provide their LDAP credentials to log in to the system. Once authenticated, Bigbytes will use the authorization filter to determine the user's permissions based on their LDAP group membership.
