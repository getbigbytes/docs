...
icon: "passport"
description: "Create users, manage users, and require sign in to authenticate and use Bigbytes."
---
...
If you’re using Bigbytes version 0.8.4 or greater, than you can optionally turn on user authentication.
...
You can set the `BIGBYTES_ACCESS_TOKEN_EXPIRY_TIME` environment variable to customize the expiration time
of a Bigbytes access token. The value should be the length of time in seconds.

Defaults to 2592000 which is 30 days.

|   |   |
| --- | --- |
| Name | `BIGBYTES_ACCESS_TOKEN_EXPIRY_TIME` |
| Value | `2592000` |

### Running Bigbytes in Docker

If you’re running Bigbytes using Docker, you can run Bigbytes and set the environment variable
...
### Running Bigbytes without Docker

If you installed Bigbytes using `pip`, `conda`, `poetry`, etc. and are running it using the `bigbytes start`
...
When Bigbytes starts, if there is no existing owner user, then a new user will be created with owner
...
Before using Bigbytes, users will have to sign in at `/sign-in`. If you go to a page that requires
authentication and you’re not signed in, you’ll be redirected to the sign in page.

![Sign in page](https://raw.githubusercontent.com/getbigbytes/assets/main/users/sign-in.jpg)
...
![Sign in page](https://raw.githubusercontent.com/getbigbytes/assets/main/users/users-list.jpg)
