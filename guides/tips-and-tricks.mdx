---
title: "Bigbytes tips & tricks"
sidebarTitle: "Tips & tricks"
icon: "question"
description: "Here are some neat ways you can make the most of Bigbytes! Tips and tricks to help you make the most magical pipelines possible."
---

<AccordionGroup>
<Accordion title="Version Control" icon="cat">

  Bigbytes has native version control for your files, so you'll never lose work again!
...
<Accordion title="DuckDB integration" icon="duck">
  ...
  Bigbytes integrates with DuckDB! Watch the video below to learn more or read on [here](/integrations/databases/DuckDB).
...
<Accordion title="Edit Pipelines with AI" icon="robot">
  ...
  Bigbytes has an integration with the OpenAI API. Watch the video to learn about how you can generate pipelines using the power of AI.
...
<Accordion title="Stream data to any destination" icon="water">
  ...
  Bigbytes supports streaming data to any of our destinations. Watch the video below to learn about configuring a streaming pipeline in under a minute!
...
<Accordion title="Use Split Pane View" icon="window">
  The Bigbytes pipeline editor can be used in a single, notebook-style interface _or_ in a split-pane view, with block output displayed on the right and blocks themselves on the left.

  Split pane view can be enabled by selecting the "split" icon in the pipeline editor, as shown below:

  <Frame>
    <img src="https://github.com/getbigbytes/assets/blob/main/split-pane-view.png?raw=true"/>
  </Frame>
...
<Accordion title="Group & Filter Pipelines" icon="folder">
  ...
  This week's Bigbytes tip is all about organization!
...
<Accordion title="Dynamic Blocks" icon="bomb">
  ...
  Dynamic Blocks in Bigbytes give you the power to parallelize your workflow and reduce output effortlessly. It's a pattern that has been around in data engineering, but we've made it incredibly easy and efficient.
...
<Accordion title="API Triggers" icon="code">
  Did you know you can trigger pipelines directly with API calls in Bigbytes? 🧙‍♂️💻
...
<Accordion title="Backfills" icon="truck">
  Managing pipelines can be tricky, especially when it comes to backfilling missing data or re-running pipelines. But with Bigbytes, backfills are a breeze! No need for custom scripts or complex setups. 🙌
  
  You can easily define a backfill for a specific date range. Whether you're missing data or want to add new columns, Bigbytes has got you covered. Simply specify the start and end dates, and let Bigbytes do the rest.
  ...
  To learn more about backfills using Bigbytes, check out our documentation [here](/orchestration/backfills/overview).
