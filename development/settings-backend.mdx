---
title: "Configuring Bigbytes settings backend"
...
Available in Bigbytes versions >= 0.9.66

Not all Bigbytes settings can be read through a settings backend. You can find a list of settings
that can be read through a settings backend [here](https://github.com/getbigbytes/bigbytes/blob/master/bigbytes/settings/keys/__init__.py).

When any settings backend is enabled, Bigbytes will first attempt to read the settings value
from the settings backend before falling back to environment variables.

</Note>

By default, many of Bigbytes's configurable settings are read from environment variables. This may make sense
...
When the AWS Secrets Manager settings backend is enabled, Bigbytes will read the value from the
AWS secret name: `{prefix}{settings_name}`. For example if the setting is `OKTA_CLIENT_SECRET`
and the prefix is `default_repo/settings/`, Bigbytes will fetch the value for `default_repo/settings/OKTA_CLIENT_SECRET`.
