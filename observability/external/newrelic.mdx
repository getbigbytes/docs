...
An example `example_newrelic.ini` file is attacehd in Bigbytes repo under folder `integrations/newrelic`.

Add path of your newrelic configuration into environment variable `NEW_RELIC_CONFIG_PATH`.

## Using New Relic in Bigbytes

Monitoring with New Relic will be enabled the next time you start the Bigbytes server.

The following logs and errors will be reported to New Relic:

* Trigger runs.
* Bigbytes scheduler.
* `bigbytes run` CLI command.
