...
description: "A metric is a value that describes or summarizes features from a collection of data points. For example, `Num unique user ids` is a metric."
sidebarTitle: Metrics
---

It describes the unique number of `user_id`s in a collection of `user_id` data points.

In Bigbytes, metrics are used to summarize dimensions or, sometimes, other metrics.
...
To add a metric to Bigbytes using the `meta` tag, you define it in your dbt project under the dimension name you're trying to describe/summarize.
...
| --------------------------------------------------- | -------- | ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| label                                               | No       | string                         | Custom label. This is what you'll see in Bigbytes instead of the metric name.                                                                                                                                                                       |
| [type](#metric-types)                               | Yes      | metric type                    | Metrics must be one of the supported types.                                                                                                                                                                                                          |
| [description](#adding-your-own-metric-descriptions) | No       | string                         | Description of the metric that appears in Bigbytes. A default description is created by Bigbytes if this isn't included                                                                                                                            |
| [sql](#using-custom-sql-in-aggregate-metrics)       | No       | string                         | Custom SQL used to define the metric.                                                                                                                                                                                                                |
| hidden                                              | No       | boolean                        | If set to true, the metric is hidden from Bigbytes. By default, this is set to false if you don't include this property.                                                                                                                            |
...
| [urls](/references/dimensions#urls)                 | No       | Array of \{ url, label }        | Adding urls to a metric allows your users to click metric values in the UI and take actions, like opening an external tool with a url, or open at a website. You can use liquid templates to customise the link based on the value of the dimension. |
| [show\_underlying\_values](#show-underlying-values) | No       | Array of dimension names       | You can limit which dimensions are shown for a field when a user clicks View underlying data. The list must only include dimension names from the base model or from any joined models.                                                              |
...
<Frame>
  <img src="/images/references/underlying-data-ec265ef84085672491f477d180d173bc.jpg"/>
</Frame>
...
Filters are applied to metrics any time that metric is used in Bigbytes. Filters can only be used with [aggregate metric types](#metric-categories).
...
To filter a field by a set of given values you can supply them as an array for that field. For example, if you wanted to filter for orders with order status `completed` or `shipped` you should write the metric like:
...
You can use the `format` parameter to have your metrics show in a particular format in Bigbytes.
