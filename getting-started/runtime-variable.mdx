...
You can create new global variables from the Bigbytes UI through the "Variables" tab
...
![Create Global Variables](https://github.com/getbigbytes/assets/blob/main/configure-runtime-variables.png?raw=True)

### In code

You can also create pipeline-level variables in code by editing the pipeline's `metadata.yaml` file.

![Create Pipeline Variables](https://github.com/getbigbytes/assets/blob/main/configure-variables-in-code.png?raw=True)
...
A big thanks to community member [Preston Dotsey](https://bigbytes.slack.com/team/U061CMVAJH1) for their contribution!

### In Python code
You can also save the variable in Python code. One example usage is to save checkpoint data. 

```python
from bigbytes.data_preparation.variable_manager import set_global_variable
set_global_variable(pipeline_uuid, key, value)
```

## Default Runtime Variables

Bigbytes provides some default variables to give context on the pipeline execution.
...
```bash
bigbytes run <project_path> <pipeline_uuid> --runtime-vars '{"name": "value"}'...

# example with 2 runtime variables "name" and "ds":
bigbytes run default_repo default_pipeline --runtime-vars '{"name": "default", "ds": "2022-08-18"}'
```
...
arguments to `bigbytes.run()`:

```python
bigbytes.run('sample_pipeline', 'repos/default_repo', filepath = 'path/to/my/file.csv', row_limit=1000)
```
...
Using Bigbytes's runtime variables this is made a very simple task:
...
`bigbytes.run()`, providing the current date and log folder as keyword arguments:

```python
bigbytes.run('log_stats_ingestion', 'repos/default_repo', current_date='2022-07-29', log_folder='logs/webapp')
```
...
`bigbytes.run()`. Specify values for runtime variables as keyword arguments to
this function:

```python
bigbytes.run('model_rocket_data_ingestion', 'repos/default_repo', pi=3.1415, conversion_factor=3.6)
```
...
```python
bigbytes.run('model_rocket_data_ingestion', 'repos/default_repo', pi=3.1415, conversion_factor=2.24)
```
