...
<video
    ...
    src="https://github.com/getbigbytes/assets/raw/main/version-control/demo.mp4">
</video>

## Setup

<Frame>
    <img alt="Setup" src="https://github.com/getbigbytes/assets/blob/main/version-control/setup.png?raw=true" />
</Frame>
...
authenticate with multiple git providers if you need to. Bigbytes will attempt to use the oauth provider
based on the URL of the remote.

<Warning>

Bigbytes authenticates with a default GitHub OAuth application that may not be able to access
...
steps in the `Authenticate with GitHub or GHE` section below to connect Bigbytes with your OAuth
application.

</Warning>

When creating the OAuth app for GHE, BitBucket, GitLab, you will need to set the authorized
callback URL to `http(s)://<your-bigbytes-url>/oauth`.
...
<Step title="Register Bigbytes in GitHub">

In order for Bigbytes to authenticate with your GitHub host, you will need to add Bigbytes as an
...
You will need to set the following environment variables in your Bigbytes environment:
...
<Step title="Add Bigbytes as an OAuth consumer">

In order for Bigbytes to authenticate with your BitBucket, you will need to add Bigbytes as an
...
<Step title="Authenticate Bigbytes with Bitbucket">
...
Bigbytes will request `read_user`, `write_repository`, and `api` scopes from GitLab in order
...
<Step title="Add Bigbytes Oauth application">

In order for Bigbytes to authenticate with your GitLab, you will need to add a Bigbytes oauth application.
...
<Step title="Authenticate Bigbytes with GitLab">
...
First, you will need to enable the [Active Directory](https://docs.getbigbytes.com/production/authentication/microsoft) integration in order for Bigbytes to authenticate your user.
...
<Step title="Authenticate Bigbytes with Azure DevOps">
...
<Frame>
    <img alt="Authenticate" src="https://github.com/getbigbytes/assets/blob/main/version-control/authentication.png?raw=true" />
</Frame>

After authorizing Bigbytes, you’ll be redirected back to the initial page and an access token will
...
For example, the remote URL for Bigbytes is `https://github.com/getbigbytes/bigbytes.git`.
...
<Frame>
    <img alt="Branches" src="https://github.com/getbigbytes/assets/blob/main/version-control/branches.png?raw=true" />
</Frame>
...
<Frame>
    <img alt="Files" src="https://github.com/getbigbytes/assets/blob/main/version-control/files.png?raw=true" />
</Frame>
...
<Frame>
    <img alt="Push" src="https://github.com/getbigbytes/assets/blob/main/version-control/push.png?raw=true" />
</Frame>
