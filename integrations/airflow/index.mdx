---
title: "Run Bigbytes pipelines in Airflow"
sidebarTitle: "Overview"
icon: "wind-turbine"
---

Need help integrating Bigbytes into an existing Airflow project?

Check out this [tutorial](/guides/integrate-bigbytes-airflow) or
get instant help from us in [Slack](https://www.getbigbytes.com/chat).

---

We support running the pipeline in Airflow DAGs.
* You need to firstly install `bigbytes` library by adding `bigbytes` to your requirements.txt file.
* Then you need to download the bigbytes pipeline code into your Airflow directory. You can achieve it by using a git submodule in your Airflow directory.
* In your Bigbytes project's metadata.yaml file, please specify a variables_dir that you want to store the output of each block execution (You need to have write permission to the variables_dir). Example: `variables_dir: /tmp`.

We provide multiple ways to run bigbytes pipelines in Airflow.
- [Create DAGs for all the pipelines in Bigbytes project](#create-dags-for-all-the-pipelines-in-bigbytes-project)
...
## Create DAGs for all the pipelines in Bigbytes project
```python
...
dag = DAG(
    ...
    description='Test running bigbytes pipeline.',
...
task = BashOperator(
    ...
    bash_command=f'bigbytes run {project_path} {pipeline_name}',
)
```
