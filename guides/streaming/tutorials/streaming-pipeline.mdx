...
9. Send messages to the topic named `test` by typing the following JSON strings in the
   terminal (Note that Kafka messages in Bigbytes are assumed to be in JSON format):
   ```bash
   >{ "hello": 1 }
   >{ "this is a test": 1 }
   >{ "test": 1 }
...
Start Bigbytes using Docker. Run the following command to run Docker in network
mode:
...
docker run -it -p 6789:6789 -v $(pwd):/home/src \
  ...
  getbigbytes/bigbytes /app/run_app.sh  bigbytes start default_repo
```
...
If you can’t connect to Kafka locally in a Docker container using Bigbytes in a
Docker container, do the following:

1. Clone Bigbytes: `git clone https://github.com/getbigbytes/bigbytes.git`.
2. Change directory into Bigbytes: `cd bigbytes`.
3. Edit the `docker-compose.yml` file to match this:
   ```yaml
   version: '3'
   services:
...
This will run Bigbytes in development mode; which runs it in a Docker container
using `docker compose` instead of `docker run`.

#### Using Kafka without a Docker container

Start Bigbytes using Docker. If you haven’t done this before, refer to the
...
3. Add a data loader block, select `Kafka`, and paste the following:
   ...
   1. By default, the `bootstrap_server` is set to `localhost:9092`. If you’re
      running Bigbytes in a docker container, the `bootstrap_server` should be
      `kafka:9093`.
...
Once you run the code snippet above, go back to your streaming pipeline in Bigbytes
