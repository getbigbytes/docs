...
[./gcp/variables.tf](https://github.com/getbigbytes/bigbytes-terraform-templates/blob/master/gcp/variables.tf)
file.
...
Google Cloud Run supports pulling Docker images directly from Docker Hub. To use the latest Bigbytes image (default):
...
Simply change the tag to adjust the version of Bigbytes you want to deploy. If you need to build your own image, you can push it to Docker Hub or follow our guide to use [GCP Artifact Registry](/production/deploying-to-cloud/gcp/gcp-artifact-registry).
...
[./gcp/main.tf](https://github.com/getbigbytes/bigbytes-terraform-templates/blob/master/gcp/main.tf):
...
3.  Under the field labeled **New principles**, add the service account that is
    ...
    <Note>
      ...
      [`main.tf`](https://github.com/getbigbytes/bigbytes-terraform-templates/blob/master/gcp/main.tf#L99):
...
1. Go to your Google Cloud Run dashboard, find the service running Bigbytes, and
...
1. Follow these
   ...
   to manually mount the secret to the running Bigbytes Cloud Run service and grant
...
Pull the Bigbytes Docker image using this command:
...
[this example](https://github.com/getbigbytes/bigbytes-terraform-templates/blob/master/gcp/load_balancer.tf#L23-L33)
...
To enable HTTPS for Bigbytes app deployed on GCP, you need to firstly make sure you
...
5. Visit the domain url to access your Bigbytes app.
