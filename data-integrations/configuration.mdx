...
| `env` | Environment Bigbytes is running in. | `dev`, `prod` |
| `execution_partition` | The current partition of the execution: datetime in this format `%Y%m%dT%H%M%S`. | `20231225T122520` |

### Common prefixes

Bigbytes supports table prefixes for integration sources. To add a prefix, add the following to the source configuration for a data integration pipeline:
...
<img
     alt="Configure data integrations"
     src="https://github.com/getbigbytes/assets/blob/main/integration-prefix.png?raw=True"
  />
</Frame>
...
```
{{ bigbytes_secret_var('your_secret_name') }}
```
