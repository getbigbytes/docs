In various surfaces in Bigbytes, you may be asked to input config for certain integrations
...
we created a way to store your secrets in the Bigbytes database.

<Warning>
  Your secrets are encrypted before being stored into the Bigbytes database. The encryption key
  will be stored in the Bigbytes data folder which by default will be created at `~/.bigbytes_data`. If
...
![Create Secrets](https://github.com/getbigbytes/assets/blob/main/create-secrets.png?raw=True)
...
Secrets are stored to the Bigbytes database. If you are using the same database across multiple
Bigbytes environments, i.e. development and production, the secrets will not be shared across the environments
unless you shared the same Bigbytes data directory for every environment.
...
You can use the following syntax to have Bigbytes interpolate the secret when reading from the config:

```yaml
field_name: "{{ bigbytes_secret_var('your_secret_name') }}"
```

![Using Secrets](https://github.com/getbigbytes/assets/blob/main/using-secrets.png?raw=True)

You can also fetch the secret value in a Bigbytes code block by importing a helper method:
