...
description: "Setup your Bigbytes project respository."
icon: "list-check"
---

1.  Create a parent folder for your Bigbytes project (e.g. `my_team`).
2.  Change directory (e.g. `cd my_team`) into the parent folder and start Bigbytes
    locally: For example:
...
3.  Once you’re done developing, copy the contents of the this
    [Dockerfile template](https://github.com/getbigbytes/docker/blob/master/Dockerfile)
    and paste it into a new `Dockerfile` located in the parent folder of your
    Bigbytes project (e.g. `my_team/Dockerfile`). Replace all instances of the
...
4.  Your current folder structure should look like this:
    ...
    |   Dockerfile
    ```
5.  Build a custom Docker image using `getbigbytes/bigbytes:latest` as the base and
    ...
    <Note>
      Change `bigbytesprod` to any other name. You’ll need this tag name when
      deploying to production in the cloud.
    </Note>

6.  To test the new image works, run the following command:
    ```bash
    docker run -it -p 6789:6789 bigbytesprod:latest /app/run_app.sh bigbytes start demo_project
    ```
...
8.  The next steps depends on your deployment method. If you are using
    ...
    tag name (e.g. `bigbytesprod`) when pushing a Docker image to a remote Docker
    registry.
