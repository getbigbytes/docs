...
icon: "arrows-spin"
description: "⏰ Run dbt models in Bigbytes in under a minute."
---
...
Our demo dbt repo currently offers two quickstarts: one that runs a simple dbt pipeline and one that runs a dbt pipeline from an external repo. At this time, running dbt in Bigbytes requires Docker, so both of our demos are `docker compose` based.
...
The following command will clone the [demo repo](https://github.com/getbigbytes/dbt-quickstart), copy `dev.env` to `.env` and run `docker compose up` to start Bigbytes _and_ a Postgres database.

<CodeGroup>
```bash Mac/Linux
git clone https://github.com/getbigbytes/dbt-quickstart bigbytes-dbt-quickstart \
&& cd bigbytes-dbt-quickstart \
...
After running the above command, open the Bigbytes overview page at http://localhost:6789. Click the pipelines icon on the left to enter the pipelines overview.

![Open Pipelines](https://github.com/getbigbytes/assets/blob/main/dbt/setup-dbt/go-to-pipelines.gif?raw=True)
...
![Run simple pipeline](https://github.com/getbigbytes/assets/blob/main/dbt/setup-dbt/simple-pipeline.png?raw=True)
...
![Compile external pipeline](https://github.com/getbigbytes/assets/blob/main/dbt/setup-dbt/external-pipeline.png?raw=True)

You just:

- Pulled a GitHub repo and wrote it to your local Bigbytes directory
...
![Trigger dbt](https://github.com/getbigbytes/assets/blob/main/dbt/setup-dbt/trigger-dbt.gif?raw=True)
