...
description: "Debugging common issues in Bigbytes and other frequently asked questions."
---
...
To temporarily fix this, open the `/terminal` page in Bigbytes.
Then type the following command press enter to run it:

```bash
rm -rf bigbytes_data/[project_name]/pipelines/[pipeline_name]/.variables
```

<Info>Change `[project_name]` to your Bigbytes project name and change `[pipeline_name]` to the name of the pipeline that is freezing the browser.</Info>

The above command will remove the data output files for each block in your pipeline.

---

## Can’t connect to local database from within Bigbytes via Docker

If you’re running Bigbytes via Docker and you’re trying to connect to a local database outside of
...
Thankfully, [Bigbytes Community](https://www.getbigbytes.com/chat) member [Josh Pachner](https://bigbytes.slack.com/team/U05B9DLK0N7) has a solution.
...
## Bigbytes doesn't load properly when running locally

When attempting to connect to Bigbytes with a port _other_ than `6789`, the browser may not render the page correctly with `localhost:port`.

Instead, use `127.0.0.1:port` to connect to Bigbytes.
...
docker run -it -p 6789:6789
    ...
    -v $(pwd):/home/src getbigbytes/bigbytes \
    /app/run_app.sh bigbytes start [project_name]
```

or, using `docker compose`, add a volume mount to your Bigbytes service. The following is an example of mounting the `~/.aws` directory for authenticating via the AWS CLI:
...
## "Run block" button doesn't work when running Bigbytes with Nginx

If Bigbytes terminal or "Run block" button doesn't work, it usually means there's a WebSocket connection issue. If Bigbytes server is deployed with Nginx, you can add the below config to Nginx config to enable WebScoket connection.
...
If your block gets stuck in running status in the Bigbytes editor, you can try the following things:
...
When deploying your Bigbytes server on Cloud (Kubernetes, AWS, GCP, Azure, etc.) with Terraform scripts or Helm chart, there're health checks set up for the
