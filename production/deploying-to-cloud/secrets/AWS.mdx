...
sidebarTitle: "AWS"
description: "Attention Bigbyters: This classified document contains vital intel on securing sensitive data within the AWS Secrets Manager vault. Study the protocols carefully to create and operationalize confidential secrets, granting you clearance to integrate them into your Bigbytes data pipeline projects."
icon: "aws"
---
![It's Classified](https://github.com/getbigbytes/assets/blob/main/secrets_assets/Source_%20Giphy%20(4).gif?raw=True)
### **Creating a New Secret in AWS Secrets Manager**

Before you can use AWS Secrets Manager with Bigbytes, you need to create a new secret in the AWS Secrets Manager console. Follow these steps:
...
Your new secret is now stored in AWS Secrets Manager. Make a note of the secret's ARN (Amazon Resource Name), as you'll need it to access the secret from Bigbytes.
...
### Working with Secrets in Bigbytes

Bigbytes works with AWS secrets in two ways: directly accessing them through Python code, and referencing them in YAML configuration files using placeholders that Bigbytes resolves at runtime.

**Python:**

To use secrets in Python code, read these [**instructions**](https://docs.getbigbytes.com/design/data-loading#aws-secret-loader).

**YAML:**
<Accordion title="Add Secrets Through Bigbytes UI">
Bigbytes allows you to add your secrets directly through the UI. Here’s how:
...
![Secrets UI](https://github.com/getbigbytes/assets/blob/main/secrets_assets/secrets_aws.png?raw=True)
3. Make the changes as pictured below to represent your secrets in the io_config.yml file
![io_config](https://github.com/getbigbytes/assets/blob/main/secrets_assets/io_config_aws.png?raw=True)
4. Each secret is now stored and referenced for configuration to AWS

By following these steps, you can securely store and reference your AWS credentials and configuration through the Bigbytes UI, allowing you to seamlessly integrate with AWS services like Secrets Manager throughout your data pipelines.
</Accordion>
...
![Shhhh](https://github.com/getbigbytes/assets/blob/main/secrets_assets/Source_%20Giphy%20(5).gif?raw=True)
...
By integrating with AWS Secrets Manager, you can ensure that your sensitive data is securely stored and accessed throughout your Bigbytes data pipelines.
