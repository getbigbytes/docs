...
The most common AWS deployment of Bigbytes is with AWS ECS. Our Terraform scripts manage the process of creating an ECS cluster, a load balancer, and a database. [Here's a link](/production/deploying-to-cloud/aws/resources) to the default output of `terraform plan` on AWS ECS.

<Frame height="400" caption="An architecture diagram of a Bigbytes AWS ECS deployment.">
  <img alt="Contributing" src="https://github.com/getbigbytes/assets/blob/main/architecture/aws-ecs.png?raw=True" />
</Frame>
...
The Bigbytes application runs as multiple containers in an Amazon ECS service.
...
We use a Postgres RDS to hold the the backend database for your Bigbytes instance. This database contains the metadata that powers Bigbytes. Read more [here](/design/abstractions/project-structure#project-data).
...
The Bigbytes application, ALB, ECS, EFS, and RDS are all deployed within a VPC in our Terraform scripts.
