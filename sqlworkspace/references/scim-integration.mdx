...
SCIM Integration is only available on Bigbytes Enterprise plans. For more information on our plans, visit our [pricing page](https://www.getbigbytes.com/pricing).
</Info>
...
## SCIM Setup within Bigbytes

1. Sign into your Bigbytes instance, click your initials at the top-right, and select **User Settings**.
<Frame>
![](/images/references/scim-bigbytes-user-settings-9f554881763a721af6463096fe8e1d59.png)
</Frame>

2. In the sidebar, select **SCIM Access Tokens**.
<Frame>
![](/images/references/scim-bigbytes-tokens-list-b060b94f0b68fc67274186809043c175.png)
</Frame>

3. Click **Generate new token**.
   * Give it a name and an optional expiration date.
<Frame>
![](/images/references/scim-bigbytes-generate-token-fd03c005c465260741db9f144782d7e4.png)
</Frame>

4. Once generated, copy and save it in a safe place, as it cannot be viewed again once the modal is closed.
<Frame>
![](/images/references/scim-bigbytes-generated-token-851f5ec6141dbd3dcb35910687f579fa.png)
</Frame>
...
<Frame>
![](/images/references/scim-bigbytes-view-url-3cfc6eb4588ba52111f287b6a5f08603.png)
</Frame>
...
### Step 2 - Connect SCIM to Bigbytes from Okta
...
5. Fill in the following fields:
   ...
   * **API Token**: `Bearer YOUR_SCIM_TOKEN` (See SCIM Setup within Bigbytes above for generating a token)
...
7. More options should be available. In **Provisioning > To App**, select **Edit**.
   ...
   * Click **Save**.
8. Test the integration by clicking **Assignments**. Select **Assign > Assign to people**. Choose a user and click **Assign > Save and Go Back**. This user should be created in your Bigbytes instance.
<Frame>
...
Bigbytes will sync the active status from Okta to Bigbytes. For example, if a user is provisioned as inactive or is deactivated in Okta, that user will still exist in Bigbytes marked as inactive, meaning they will be unable to use the platform.
...
### Step 1 - Connect SCIM to Bigbytes from Azure
...
### Step 2 - Connect SCIM to Bigbytes from Azure
...
Bigbytes will sync the active status from Azure to Bigbytes. For example, if a user is provisioned as inactive or is deactivated in Azure, that user will still exist in Bigbytes marked as inactive, meaning they will be unable to use the platform.
</Note>
...
```json
curl --location --request PATCH 'https://my.getbigbytes.com/api/v1/scim/organization-access-tokens/<personal-access-token-uuid>/rotate' \
