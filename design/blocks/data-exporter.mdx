...
<img
  alt="exporter"
  src="https://github.com/getbigbytes/assets/blob/main/blocks/exporter/data-exporter.gif?raw=True"
/>
Bigbytes natively supports integration with a variety of data storage systems. However, these integrations often
...
2. **Supported Data Storage Systems**:
    - Bigbytes supports a wide range of storage systems including PostgreSQL, MySQL, AWS S3, Google Cloud Storage, Azure Blob Storage, and many more.
...
Configure the `io_config.yml` file to connect your Bigbytes pipeline to a snowflake data warehouse. While optional,
depending on how your Snowflake DW is configured you may need to enter all information into the .yml file. 
It’s recommended to store sensitive information as Secrets. See the general [Secrets documentation](https://docs.getbigbytes.com/development/variables/secrets)
...
def export_data_to_snowflake(df: DataFrame, **kwargs) -> None:
    ...
    Docs: https://docs.getbigbytes.com/design/data-loading#snowflake
    """
...
Configure the `io_config.yml` file to connect your Bigbytes pipeline to a Azure Blob Storage. Configure some Secrets and enter them into `io_config.yml` file.
If you need more information on entering secrets see this [documentation](https://docs.getbigbytes.com/production/deploying-to-cloud/secrets/Azure).
...
def export_data_to_azure_blob_storage(df: DataFrame, **kwargs) -> None:
    ...
    Docs: https://docs.getbigbytes.com/design/data-loading
    """
    config_path = path.join(get_repo_path(), 'io_config.yaml')
    config_profile = 'default'
...
Configure the `io_config.yml` file to connect your Bigbytes pipeline to a PostgreSQL database. Configure some Secrets and enter them into `io_config.yml` file.
If you need more information on entering secrets see this [documentation](https://docs.getbigbytes.com/production/deploying-to-cloud/secrets/Azure).
...
def export_data_to_postgres(df: DataFrame, **kwargs) -> None:
    ...
    Docs: https://docs.getbigbytes.com/design/data-loading#postgresql
    """
...
<Note>
Remember Secrets can be stored in Bigbytes’s internal Secrets Manager, .YAML files, or sync directly with Cloud Secret Managers.
</Note>
