...
description: "User attributes provide a way to customise Bigbytes charts, dashboards, and other behaviour depending on the user that is logged in. For example, you could only show your sales team members data that are relevant to their region."
---
...
## Using user attributes in Bigbytes

There are several places in Bigbytes where you can customise behaviour based on user attributes.

When referencing user attributes in SQL you can use the following [SQL variables](/references/sql-variables):

* `${bigbytes.attributes.my_attr_1}` \- a user attribute called `my_attr_1`
   * (optional) `ld` as an alias for `bigbytes`
   * (optional) `attribute` or `attr` as an alias for `attributes`
* `${bigbytes.user.<intrinsic_attribute>}` \- reference an `intrinsic_attribute` of the current Bigbytes user
   * (optional) `ld` as an alias for `bigbytes`
...
models:
  - name: example
    meta:
      sql_filter: ${bigbytes.user.email} = 'example@getbigbytes.com'
```

If you are self hosting you can enable [SMTP](/self-host/customize-deployment/configure-smtp-for-bigbytes-email-notifications) or [SSO](/self-host/customize-deployment/use-sso-login-for-self-hosted-bigbytes) authentication to allow users to verify their email address.
</Note>

### Row filtering with `sql_filter`

You can use user attributes to filter the rows returned by a query. This is useful if you want to restrict the data based on the user's attributes. To reference a user attribute in your sql, use the special bigbytes reference`${bigbytes.attributes.<attribute_name> }`. You should use the `IN` operator since the attribute might have multiple values.
...
models:
  - name: my_model
    meta:
      sql_filter: ${TABLE}.sales_region IN (${bigbytes.attributes.sales_region})
```
...
1. **Multiple attributes joined with `AND`.** In the example below, only users with `is_admin: "true" AND team_name: "HR"` have access to the `salary` dimension in Bigbytes.
...
1. **Multiple attribute values joined with `OR`.** In the example below, users with `team_name = 'HR' OR team_name = 'C-Suite'` have access to the `salary` dimension in Bigbytes.
...
You can use user attributes to filter the rows returned by a join. This is useful if you want to restrict the data returned from the joined table. To reference a user attribute in your sql, use the special bigbytes reference`${ bigbytes.attributes.<attribute_name> }`. For example, if you have a user attribute called `sales_region` you can use it in your sql like this:
    ...
    meta:
      joins:
        - join: joined
          sql_on: >
            ${base}.id = ${joined}.id
            AND ${joined}.sales_region = ${bigbytes.attributes.sales_region}
```
...
<iframe width="100%" height="420" src="https://www.youtube.com/embed/t6QS0rr0PDQ" title="Customizing Bigbytes with User Attributes" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
models:
  - name: my_model
    meta:
      sql_filter: ${TABLE}.fruit IN (${bigbytes.attributes.fruit})
```

In this example, the `${bigbytes.attributes.fruit}` will be replaced with `'kiwi','orange','coconut'`. The final SQL will be `my_model.fruit IN ('kiwi','orange','coconut)`
