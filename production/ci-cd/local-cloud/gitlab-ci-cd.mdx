...
1. Create a new file in your repository and name it `.gitlab-ci.yml`.
   If you followed the [Bigbytes project setup instructions](/production/ci-cd/local-cloud/repository-setup),
   then your current folder structure could look like this:
    ...
    |   Dockerfile
    ```
1. Copy the contents of this [GitLab CI/CD template file](https://github.com/getbigbytes/bigbytes/blob/master/templates/gitlab_cicd/aws.gitlab-ci.yml)
...
1. Change the following values in the file named `.gitlab-ci.yml` under the key labeled `variables`:
    ...
    | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
    | `REPOSITORY_URL`      | The URL of the AWS ECR repository you created to store your Docker images. | `679812356111.dkr.ecr.us-west-2.amazonaws.com/bigbytes-data` |
    | `CLUSTER_NAME`         | The name of your AWS ECS cluster. | `bigbytes-production-cluster` |
    | `SERVICE_NAME`         | The name of your AWS ECS service. | `bigbytes-production-ecs-service` |
    | `TASK_DEFINITION_NAME` | Go to your AWS ECS task definition for the above service. Use the name of that task definition. | `bigbytes-production-task` |
