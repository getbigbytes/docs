...
Before you configure your action, follow the [Bigbytes project setup instructions](/production/ci-cd/local-cloud/repository-setup).
...
13. Copy the contents from the GitHub Action YAML file for AWS at
    [templates/github_actions/build_and_deploy_to_aws_ecs.yml](https://github.com/getbigbytes/bigbytes/blob/master/templates/github_actions/build_and_deploy_to_aws_ecs.yml),
...
| `ECS_SERVICE`         | The name of your AWS ECS service.                                                                                                                                                                                                                                                  | `bigbytes-production-ecs-service`        |
...
3. In the field labeled **Secret**, enter the JSON string containing your GCP
   ...
   `json { "type": "service_account", "project_id": "bigbytes-123456", "private_key_id": "...", "private_key": "-----BEGIN PRIVATE KEY-----\n...\n-----END PRIVATE KEY-----\n", "client_email": "bigbytes@bigbytes-123456.iam.gserviceaccount.com", "client_id": "...", "auth_uri": "https://accounts.google.com/o/oauth2/auth", "token_uri": "https://oauth2.googleapis.com/token", "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs", "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/bigbytes%40bigbytes-123456.iam.gserviceaccount.com" } `
...
8. Copy the contents from the GitHub Action YAML file for GCP at
   [templates/github_actions/build_and_deploy_to_gcp_cloud_run.yml](https://github.com/getbigbytes/bigbytes/blob/master/templates/github_actions/build_and_deploy_to_gcp_cloud_run.yml),
   and paste it into the textarea.
9. Change the following values under the key labeled `env`:
   ...
   | Key                             | Description                                                                                 | Sample value  |
   | ------------------------------- | ------------------------------------------------------------------------------------------- | ------------- |
   | `GAR_LOCATION`                  | Region that Bigbytes is already deployed in.                                                    | `us-east1`    |
   | `GOOGLE_CLOUD_RUN_SERVICE_NAME` | The name of your Google Cloud Run service. Go to the Google Cloud Run dashboard to find it. | `bigbytes-data`   |
   | `IMAGE`                         | The name of the Docker image you pushed to the above GCP Artifact Registry.                 | `bigbytes`      |
   | `PROJECT_ID`                    | Project ID of where you launched Bigbytes using Terraform.                                      | `bigbytes-123456` |
   | `REPOSITORY`                    | The name of your GCP Artifact Registry that is storing your Docker image.                   | `bigbytesprod`    |
...
Bigbytes can be deployed to Azure using GitHub Actions.

Our template for deploying to an Azure Container instance is located [here](https://github.com/getbigbytes/bigbytes/blob/master/templates/github_actions/build_and_deploy_to_azure_container_instance.yml) and based off the "GitHub Workflow" option in Azure's docs.
