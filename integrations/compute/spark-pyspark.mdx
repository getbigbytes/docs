...
icon: "sparkles"
description:
  "This is a guide for using Spark (PySpark) with Bigbytes in different cloud
...
When you run Bigbytes and Spark in the same Kubernetes cluster, you can set the environment variable `SPARK_MASTER_HOST`
to the url of the master node of the Spark cluster in Bigbytes container. Then you'll be able to connect Bigbytes to your Spark
cluster and execute PySpark code in Bigbytes.

Here is an overview of the steps required to use Bigbytes with Spark in Kubernetes cluster.
...
### 2. Build Bigbytes docker image with Spark environment
* Download the [Dockerfile template](https://github.com/getbigbytes/bigbytes/blob/master/integrations/spark/Dockerfile) for Bigbytes with Spark environment.
...
### 3. Run Bigbytes in Kubernetes cluster with the `SPARK_MASTER_HOST` environment variable
...
* Then deploy Bigbytes to Kubernetes with the updated yaml file.
...
Updates
* In Bigbytes version `0.8.83` and above, you don't need to specify the environment variable anymore.
...
### 4. Run PySpark code in Bigbytes
When Bigbytes pod is running, you can create a "Standard (batch)" pipeline (`python` kernel) and then write PySpark code in any blocks.
...
### 1. Build Bigbytes docker image with Spark environment
* Download the [Dockerfile template](https://github.com/getbigbytes/bigbytes/blob/master/integrations/spark/Dockerfile) for Bigbytes with Spark environment.
* Change the current directory to the directory that contains the `Dockerfile`. Build the docker image with the command `docker build -t mage_spark .`

### 2. Start Bigbytes with `SPARK_MASTER_HOST` environment variable
Type this command in your terminal to start Bigbytes using docker

```bash
docker run -it --name bigbytes_spark -e SPARK_MASTER_HOST='local' -p 6789:6789 -v $(pwd):/home/src bigbytes_spark \
  /app/run_app.sh bigbytes start demo_project
```
Notes
...
### 3. Run PySpark code in Bigbytes
When Bigbytes is running, you can create a "Standard (batch)" pipeline (`python` kernel) and then write PySpark code in any blocks.
...
### 2. Start Bigbytes using docker with the following command in your terminal

```bash
docker run -it --name bigbytes_spark -p 6789:6789 -v $(pwd):/home/src bigbytes_spark \
  /app/run_app.sh bigbytes start demo_project
```
Notes
* `demo_project` is the name of your project, you can change it to anything you want
...
### 4. Run PySpark code in Bigbytes
When Bigbytes is running, you can create a "Standard (batch)" pipeline (`python` kernel), and add a block,
...
```python
from bigbytes.data_preparation.repo_manager import RepoConfig, get_repo_path
from bigbytes.services.spark.config import SparkConfig
from bigbytes.services.spark.spark import get_spark_session
...
### 1. Build Bigbytes docker image with a Hadoop Yarn environment
* Download the [Dockerfile template](https://github.com/getbigbytes/bigbytes/blob/master/integrations/hadoop/Dockerfile) for Bigbytes with Hadoop environment.
* Build the docker image with the command `docker build -t mage_hadoop .`


### 2. Start Bigbytes with the following command in your terminal using docker

```bash
docker run -it --name bigbytes_hadoop -p 6789:6789 -v $(pwd):/home/src bigbytes_hadoop \
  /app/run_app_with_hadoop.sh bigbytes start demo_project
```
Notes
* `demo_project` is the name of your project, you can change it to anything you want

### 3. Run PySpark code in Bigbytes using Hadoop Yarn
...
Here is an overview of the steps required to use Bigbytes locally with Spark in AWS:

1. [Create an S3 bucket for Spark](#1-create-an-s3-bucket-for-spark)
2. [Start Bigbytes](#2-start-bigbytes)
...
these steps, please join our [Slack](https://www.getbigbytes.com/chat)
...
Using Bigbytes with Spark is much easier if you use Docker.

Type this command in your terminal to start Bigbytes using docker (Note:
`demo_project` is the name of your project, you can change it to anything you
want):

```bash
docker run -it -p 6789:6789 -v $(pwd):/home/src getbigbytes/bigbytes \
  /app/run_app.sh bigbytes start demo_project
```
...
Once you’ve acquired those credentials, you can switch your kernel in your Bigbytes notebook
to `pyspark` kernel. Bigbytes will automatically creates the EMR cluster when you switch to `pyspark`
...
This will allow your locally running Bigbytes to remotely access AWS EMR.

### 6. Sample pipeline with PySpark code

If you aren’t using Docker and you installed Bigbytes using `pip`, you must run the
...
def data_from_internet():
    url = 'https://raw.githubusercontent.com/getbigbytes/datasets/master/restaurant_user_transactions.csv'
...
def export_data(df: DataFrame, **kwargs) -> None:
    (
        ...
        .option('header', 'True')
        .mode('overwrite')
        .csv('s3://bigbytes-spark-cluster/demo_project/demo_pipeline/')
    )
```
...
def load_data(**kwargs) -> DataFrame:
    df = (
        ...
        .option('delimiter', ',')
        .load('s3://bigbytes-spark-cluster/demo_project/demo_pipeline/*')
    )
...
Make sure the EMR cluster is accessible from the Bigbytes server. You can verify whether Bigbytes server's
