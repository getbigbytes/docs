...
<Warning>
    **Important**
    ...
    `docker run -d --env-file .env -e SERVER_LOGGING_TEMPLATE=$'%(asctime)s\t\t[%(name)25.25s]\t%(levelname)5s: %(message)s' getbigbytes/bigbytes:latest`
</Warning>
...
Then you can run command `bigbytes clean-old-logs [project_path]` to clean up old logs
