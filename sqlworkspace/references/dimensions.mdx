...
For a dimension to appear in Bigbytes, you just need to declare it in your dbt model's YAML file.
...
models:
  - name: my_model
    columns:
      - name: user_id # will be "User id" in BigBytes
    ...
    columns:
      - name: revenue_gbp_total_est
        ...
        meta:
          dimension:
            type: string
            label: 'Total revenue' # this is the label you'll see in Bigbytes
            description: 'My custom description' # you can override the description you'll see in Bigbytes here
...
| -------------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| label                                        | No       | string                                                                                                                                          | Custom label. If you set this property, this is what you'll see in Bigbytes instead of the dimension name.                                                                                                                                                |
| [type](#type)                                | No       | Dimension type                                                                                                                                  | The dimension type is automatically pulled from your table schemas in Bigbytes but you can override the type using this property.                                                                                                                         |
| description                                  | No       | string                                                                                                                                          | Description of the dimension in Bigbytes. You can use this to override the description you have for the dimension in dbt.                                                                                                                                 |
| sql                                          | No       | string                                                                                                                                          | Custom SQL applied to the column used to define the dimension.                                                                                                                                                                                             |
| [time\_intervals](#time-intervals)           | No       | 'default' or OFF or an array\[\] containing elements of [date](#date-options), [numeric](#numeric-options) or [string](#string-options) options | 'default' (or not setting the time\_intervals property) will be converted into \['DAY', 'WEEK', 'MONTH', 'YEAR'\] for dates and \['RAW', 'DAY', 'WEEK', 'MONTH', 'YEAR'\] for timestamps; if you want no time intervals set 'OFF'.                         |
| hidden                                       | No       | boolean                                                                                                                                         | If set to true, the dimension is hidden from Bigbytes. By default, this is set to false if you don't include this property.                                                                                                                               |
...
| [required\_attributes](#required-attributes) | No       | Object with \{ user_attribute, value }                                                                                                          | Limits access to users with those attributes                                                                                                                                                                                                               |
| [colors](#colors)                            | No       | Object with \{ value, color }                                                                                                                    | Color for the values in the chart                                                                                                                                                                                                                          |

## Type

The types of your dimensions are pulled from your data warehouse, automatically. You can override these types using the `type` meta tag in your .yml file. If you run `bigbytes generate` to generate your .yml files, then Bigbytes will add the `type` from your data warehouse to your .yml files automatically.
...
Column descriptions in your YAML file are automatically pulled into Bigbytes and you can spot them if you hover over the dimension name <Icon icon="eyes" iconType="solid"/>

<Frame>
  <img src="/images/references/screenshot-column-descriptions-3498d7b3c09c38fda5b755ce21358b2c.png"/>
</Frame>
...
When you surround text with double or single quotes it will escape the text between so that any special characters recognized by YAML will still pass through to the Bigbytes UI.
...
When you use `>-` it allows you to type descriptions that are multiple lines long in the YAML file, but the text will be combined into a single line when parsed. The `bigbytes generate` command will automatically add this to keep YAML files easy to read.
...
- name: product_tier
  description: >-
    ...
    and it will be combined in the Bigbytes UI

```

Will appear like this in the Bigbytes UI:

```yaml
This is a longer description......that requires multiple lines and it will be combined in the Bigbytes UI

```

### Vertical bar for preserving line breaks

If you need line breaks to stay in place when they show up in the Bigbytes UI, you can use a `|` character like this:
...
And in Bigbytes UI it will appear like this:
...
Bigbytes automatically adds intervals for dimensions that are timestamps or dates, so you don't have to!
...
Bigbytes breaks this out into the default intervals automatically. So, this is how `created` appears in our Bigbytes project:
...
In this example, `created` would now appear as a single, timestamp dimension without a drop-down list of time intervals in Bigbytes:
...
If you specify time intervals manually, then this overrides the default time intervals used by Bigbytes.
...
You can use the `format` parameter to have your dimensions show in a particular format in Bigbytes.
...
Bigbytes users can interact with dimension values by clicking on them. By adding custom `urls` you can configure the actions available to your users. Like linking to external tools, or taking actions in other tools.
...
The `${ value.formatted }` will be replaced with the value of the company name in the Bigbytes UI at query run time. The `${ row.company.company_id.raw }` will be replaced with the value of the company id in the Bigbytes UI at query run time. The action will be disabled if the column "company\_id" from table "company" is not part of the query.
...
| Tag                                         | Description                                                                                  |
| ------------------------------------------- | -------------------------------------------------------------------------------------------- |
| $\{ value.formatted }                        | The exact value of the dimension as seen in the Bigbytes UI. For example "$1,427.20"        |
| $\{ value.raw }                              | The raw value of the dimension returned from the underlying SQL query. For example "1427.2 " |
| $\{ row.table\_name.column\_name.formatted } | The exact value of the column as seen in the Bigbytes UI. For example "$1,427.20"           |
| $\{ row.table\_name.column\_name.raw }       | The raw value of the dimension returned from the underlying SQL query. For example "1427.2 " |
...
| ----------- | -------------------------------------------------------------------- | ---------------------- | ------------------ |
| url\_encode | Encode a string as url safe, for example it replaces spaces with %20 | \`$\{value.formatted \\ | url\_encode }\`    |
...
Bigbytes can use `user attributes` to limit some dimensions to some users.
...
Bigbytes dimensions and custom metrics are protected by this feature, however, it is possible to write custom SQL to bypass this filter, for example:
...
A "normal" dimension is a column created in your .sql file in dbt that is written to your data warehouse. An additional dimension is not included in your dbt .sql file, so it's not written to your data warehouse. When used in Bigbytes, it just adds the dimension definition to your SQL query (so it's "created" at runtime).
