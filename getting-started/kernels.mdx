...
- Specify PySpark kernel related [metadata](#metadata) in project's
  metadata.yaml file
- Launch a remote AWS EMR Spark cluster. Install bigbytes library in bootstrap
  actions. Make sure the EMR cluster is publicly accessible.
  - You can use the `create_emr.py` script under
    [scripts/spark](https://github.com/getbigbytes/bigbytes/tree/master/scripts/spark)
...
`bigbytes run [project_name] [pipeline]` command. Example EMR config:
...
![](https://github.com/getbigbytes/assets/blob/main/switch_kernels.png?raw=True)
