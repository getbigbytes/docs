---
title: "Bigbytes environment variables"
sidebarTitle: "Environment variables"
description: "Configure Bigbytes settings through environment variables."
---

## Environment variables in Bigbytes

Environment variables may be set in your Docker container to configure Bigbytes settings. If you’re running Bigbytes using `pip` or `conda`, your local machine’s environment variables
are accessible within the running Bigbytes app.
...
If you’re running Bigbytes using Docker, you must add the following command line flags:

```bash
-e SOME_VARIABLE_NAME_1=secret_value_1 -e SOME_VARIABLE_NAME_2=secret_value_2
```

The command to run Bigbytes using Docker could look like this:
...
## Setting the prefix for Bigbytes URL

You may need to add a prefix to the Bigbytes url. You can configure the Bigbytes server to start
at a prefix to the url by setting the `BIGBYTES_BASE_PATH` environment variable.

Make sure to set the base path without the leading slash. For example, if you want to start Bigbytes at
`http://localhost:6789/base/url/path`, you would configure your environment
variable like this:

`export BIGBYTES_BASE_PATH=base/url/path`

You can also configure the base path separately for client requests and backend routes by setting
the `BIGBYTES_REQUESTS_BASE_PATH` and `BIGBYTES_ROUTES_BASE_PATH` environment variables respectively.

|                           |                                                                                     |
| ------------------------- | ----------------------------------------------------------------------------------- |
| `BIGBYTES_BASE_PATH`          | Set base path for the entire app.                                                   |
| `BIGBYTES_REQUESTS_BASE_PATH` | Set base path for frontend requests and urls, defaults to value of `BIGBYTES_BASE_PATH` |
| `BIGBYTES_ROUTES_BASE_PATH`   | Set base path for backend routes, defaults to value of `BIGBYTES_BASE_PATH`             |

## Default variables

These are the environment variables used in Bigbytes.
...
| `AUTHENTICATION_MODE`            | Set the authentication mode in Bigbytes. Omitting the variable will default to basic password authentication.                                                       | `ldap`                                                                   |
...
| `DEBUG`                          | Turn on "debug" mode. Bigbytes will output more logs that could be useful for debugging issues.                                                                     | 1                                                                        |
| `DEFAULT_EXECUTOR_TYPE`          | The default executor type to run blocks                                                                                                                         | See enum types [here](https://docs.getbigbytes.com/production/configuring-production-settings/compute-resource#2-set-executor-type-and-customize-the-compute-resource-of-the-bigbytes-executor) |
...
| `ENV`                            | Set the environment of the Bigbytes app. You can use different code or configurations in different environments.                                                    | `dev`, `staging`, `production`/`prod`, `test`                            |
...
| `BIGBYTES_ACCESS_TOKEN_EXPIRY_TIME`  | [More information](/production/authentication/overview#ldap)                                                                                                    | See link                                                                 |
| `BIGBYTES_BASE_PATH`                 | The base path or prefix of the Bigbytes server url. [More information](/getting-started/setup#setting-prefix-for-bigbytes-url)                                          | `base/path`                                                              |
| `BIGBYTES_REQUESTS_BASE_PATH`        | [More information](/getting-started/setup#setting-prefix-for-bigbytes-url)                                                                                          | `base/path`                                                              |
| `BIGBYTES_ROUTESBASE_PATH`           | [More information](/getting-started/setup#setting-prefix-for-bigbytes-url)                                                                                          | `base/path`                                                              |
| `BIGBYTES_CACHE_DIRECTORY`           | The directory Bigbytes uses for storing and retrieving temporary data used in the UI.                                                                               | `/root/.mage_data/default_repo/.cache`                                   |
| `BIGBYTES_DATABASE_CONNECTION_URL`   | Specify the database connection url for orchestration DB. Defaults to local sqlite db.                                                                          | [Example](/production/configuring-production-settings/overview#postgres) |
| `BIGBYTES_PUBLIC_HOST`               | The public host url that can be used to access the Bigbytes app. This value will be used in emails or other notifications.                                          | `http://localhost:6789`                                                  |
...
| `REQUIRE_USER_AUTHENTICATION`    | Enable user authentication in Bigbytes. [More information](/production/authentication/overview)                                                                     | 1                                                                        |
| `SERVER_VERBOSITY`               | [More information](/development/observability/logging#server-logging)                                                                                           | See link                                                                 |
| `SHELL_COMMAND`                  | Set shell command to use for the Bigbytes terminal. Default command is `bash` for macOS/Unix and `cmd` for Windows.                                                 | `bash`, `cmd`, ...                                                       |
| `ULIMIT_NO_FILE`                 | Override the maximum number of open files allowed in Bigbytes processes.                                                                                            | 8192                                                                     |
| `USER_CODE_PATH`                 | Set this path to be `/home/src/{your_project_name}` so that Bigbytes starts your project and installs the packages in requirements.txt when starting the container. | `/home/src/mage_project`                                                 |
