...
Bigbytes provides data loading clients that simplify loading and exporting data in your pipelines, allowing you to spend more time analyzing and transforming your data for ML tasks. Currently, Bigbytes includes clients for the following data sources:
...
Bigbytes's data loading clients fall into two categories:
...
While traditional Pandas IO procedures can be utilized to load files into your pipeline, Bigbytes provides the `bigbytes.io.file.FileIO` client as a convenience wrapper.
...
Loading data from a Snowflake data warehouse is made easy using the `bigbytes.io.snowflake.Snowflake` data loading client. In order to authenticate access to a Snowflake warehouse, the client requires the associated Snowflake credentials:

-   Account Username
-   Account Password
-   Account ID (including your region) (Ex: _example.us-west-2_)

These parameters can be manually specified as input to the data loading client (see [Snowflake](#snowflake) API). However we recommend using a [Configuration Loader](#configuration-settings) to handle loading these secrets. If you used `bigbytes init` to create your project repository, you can store these values in your `io_config.yaml` file and use `bigbytes.io.config.ConfigFileLoader` to construct the data loader client.
...
`bigbytes.io.redshift.Redshift`

Handles data transfer between a Redshift cluster and the Bigbytes app. Bigbytes uses temporary credentials to authenticate access to a Redshift cluster. There are two ways to specify these credentials:
...
Handles data transfer between an S3 bucket and the Bigbytes app. The `S3` client supports importing and exporting with the following file formats:
...
If an IAM profile is not set up using `aws configure`, then AWS credentials for accesses the S3 bucket can be manually specified through either Bigbytes's [configuration loader](#configuration-settings) system or through keyword arguments in the constructor (see constructor).
...
-   **Args**
    ...
    -   If IAM profile is not set up using `aws configure` and Bigbytes's configuration loader is not used, then specify your AWS credentials through the following keyword arguments:
...
`bigbytes.io.file.FileIO`

Handles data transfer between the filesystem and the Bigbytes app. The `FileIO` client currently supports importing and exporting with the following file formats:
...
`bigbytes.io.google_cloud_storage.GoogleCloudStorage`

Handles data transfer between a Google Cloud Storage bucket and the Bigbytes app. Supports loading files of any of the following types:
...
`bigbytes.io.azure_blob_storage.AzureBlobStorage`

Handles data transfer between an Azure Blob Storage conatiner and Bigbytes. Supports loading files of the following types:
-   CSV
-   JSON
-   Parquet
-   HDF5
...
`bigbytes.io.google_sheets.GoogleSheets`

Handles data transfer between a Google Sheets spreadsheet and the Bigbytes app.
...
The Google Sheets class is defined [here](https://github.com/getbigbytes/bigbytes/blob/master/bigbytes/io/google_sheets.py).

## BigQuery

`bigbytes.io.bigquery.BigQuery`

Handles data transfer between a BigQuery data warehouse and the Bigbytes app.
...
`bigbytes.io.postgres.Postgres`

Handles data transfer between a PostgreSQL database and the Bigbytes app. The `Postgres` client utilizes the following keys to connect the PostgreSQL database.
...
`bigbytes.io.mysql.MySQL`

Handles data transfer between a MySQL database and the Bigbytes app. The `MySQL` client utilizes the following keys to connect the MySQL database.
...
Handles data transfer between DuckDB database and the Bigbytes app.
...
`bigbytes.io.snowflake.Snowflake`

Handles data transfer between a Snowflake data warehouse and the Bigbytes app. The `Snowflake` client utilizes the following keys to authenticate access and connect to Snowflake servers.
...
Instead, Bigbytes provides **configuration loaders** which allow data loading clients to use your secrets without explicitly writing them in code.

<Warning>
  Currently Bigbytes only supports AWS Secrets Manager as a configuration loader. If you need to use secrets from Azure Key Vault
  or GCP Secret Manager, you can reference the following docs:
    - [Azure Key Vault](https://docs.getbigbytes.com/production/deploying-to-cloud/secrets/Azure)
    - [GCP Secret Manager](https://docs.getbigbytes.com/production/deploying-to-cloud/secrets/GCP)
</Warning>
...
```python
from bigbytes.io.config import AWSSecretLoader
...
The following are the set of allowed key names that you must name your secrets with in order for Bigbytes's configuration loaders to recognize your secrets. In code you can refer to these keys by their string name or using the `bigbytes.io.config.ConfigKey` enum. Not all keys need be specified at once - only use the keys related to the services you utilize.
...
```python
from bigbytes.io.config import ConfigKey, ConfigFileLoader
...
```python
from bigbytes.io.config import ConfigKey, EnvironmentVariableLoader
...
```python
from bigbytes.io.config import ConfigKey, AWSSecretLoader
