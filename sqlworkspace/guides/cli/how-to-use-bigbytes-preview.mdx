...
<iframe src="https://www.loom.com/embed/fddc9adab0694f60a85d0a48d67469be" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen style={{ width: '100%', height: '400px' }} ></iframe>
Preview environments will copy all spaces/charts/dashboards into your new preview environment, so you can test the content and also run [validation](/guides/cli/how-to-use-bigbytes-validate). This is only copied on preview creation, you can't sync the content afterwards.

### Run `bigbytes preview` from inside your project[](#run-bigbytes-preview-from-inside-your-project "Direct link to run-bigbytes-preview-from-inside-your-project")
...
Then `cmd` \+ `click` to open the preview link from your terminal. Once you're in Bigbytes go to `Explore` \--> `Tables`, then click on the model(s) you just updated to see your changes and play around with them.

**Problems with credentials?**

When you create developer previews, Bigbytes will use the same warehouse connection settings you have in your`profiles.yml` file for your current dbt project. This can be a problem if you're using a local database that your laptop can reach but your Bigbytes instance cannot.
...
If you've connected Bigbytes to GitHub, you can setup a `github action` and get Bigbytes to create new dynamic `preview` projects automatically when a new `pull request` is created, and it will automatically delete the `preview` project when the `pull request` is closed or merged.
...
If you haven't already set up a GitHub action for Bigbytes, you'll need to add some secrets to GitHub. If you already have a GitHub action for Bigbytes, then you can use the same Bigbytes secrets you created for your other action.
...
Now copy [this start-preview.yml file](https://github.com/getbigbytes/cli-actions/blob/main/start-preview.yml) from our [cli-actions](https://github.com/getbigbytes/cli-actions) repo

And save by clicking on `Start commit`

Do the same with [this close-preview.yml file](https://github.com/getbigbytes/cli-actions/blob/main/close-preview.yml).
...
Then, update your GitHub action to use the username as the `--target` flag for the `bigbytes start-preview` command.

```
run: bigbytes start-preview --project-dir "$PROJECT_DIR" --profiles-dir . --name ${GITHUB_REF##*/} --target ${{ github.actor }}
...
After that we need to add a new env variable to the step "Bigbytes CLI start preview" which is the schema that dbt cloud will use.
