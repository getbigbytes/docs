...
If you are using the provided Bigbytes IO libraries, you can mock the libraries in the following way:
...
class ExampleIOTest(unittest.TestCase):
    ...
    def test_pipeline_execution(self):
        ...
        with patch(
            'bigbytes.io.postgres.Postgres.with_config', return_value=postgres_mock
        ), patch('bigbytes.io.bigquery.BigQuery.with_config', return_value=bq_mock):
    ...
    def test_block_execution(self):
        ...
        with patch('bigbytes.io.bigquery.BigQuery.with_config', return_value=bq_mock):
            block1.execute_sync()
