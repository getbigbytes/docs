---
title: "Bigbytes and Native Cloud Environments"
sidebarTitle: "Deploy without Terraform"
description:
  "Just like a traditional notebook, Bigbytes supports execution in native cloud
...
Bigbytes can be run in an Amazon EC2 instance, either by
...
### Run script to start Bigbytes

We provide the `scripts/run_ec2.sh` script to launch Bigbytes in an EC2 instance.
Currently only connections via SSH are supported. To access [this script](https://github.com/getbigbytes/bigbytes/blob/master/scripts/run_ec2.sh)
clone this [repository](https://github.com/getbigbytes/bigbytes).

To run Bigbytes in an EC2 instance, you need to provide
...
2. Install Bigbytes if not already installed on instance
3. Create a default repository (default repository name is 'default_repo', use
   `--name` to specify your own custom repository name)
4. Launch the Bigbytes tool pointing at this repository

To access the Bigbytes tool, open [localhost:6789](http://localhost:6789). All
actions made here will be forwarded to your EC2 instance for execution.

To quit out of Bigbytes, stop execution in your current terminal window (Ctrl+C).
This will shutdown the Bigbytes tool alongside closing the connection to your EC2
instance.
...
You can also connect to the Bigbytes app running on your EC2 instance by editing
your security group to expose the port that Bigbytes runs on.

1. When creating your EC2 instance, edit your security group rules to allow a
   Custom TCP connection to port 6789 (the port that Bigbytes runs on).
2. Connect to your EC2 instance via SSH and install Bigbytes if you haven't already:
   ```bash
   ssh -i path/to/key/pair.pem ec2_username@ec2_public_dns_name
   pip install bigbytes
   ```
3. Start Bigbytes on your EC2 instance:

   ```bash
   bigbytes init <your_repo_name>
   bigbytes start <your_repo_name>
   ```

4. Then from your browser, access `<ec2-instance-public-ip>:6789` to access the
   Bigbytes app, where `ec2-instance-public-ip` is the Public IPv4 address of your
   EC2 instance.
...
You can also manually start a connection to the Bigbytes tool running on an EC2
instance. This process involves opening two separate SSH connections:

1. Connect to the EC2 instance to run the Bigbytes Tool. First, open an SSH
   ...
   If the Bigbytes tool is not installed, install Bigbytes using `pip`. It is
   ...
   pip3 install bigbytes
   ```

   Next, initalize your Bigbytes repository using `bigbytes init`. You can change your
   repository name as per your preference.

   ```bash
   bigbytes init default_repo
   ```

   Then start the Bigbytes app. This will start the notebook at
...
Now you can access the Bigbytes tool at [localhost:6789](http://localhost:6789) on
...
You can launch Bigbytes in an Amazon Elastic Container Service (ECS) cluster,
allowing you to perform tasks like:

- [Using the Bigbytes code editor in the ECS cluster](#running-the-bigbytes-app-in-ecs)
- [Running a Bigbytes ETL pipeline in production](#running-bigbytes-pipeline-in-ecs)

## Running the Bigbytes App in ECS

Follow the steps below to launch the Bigbytes app on your ECS cluster:

1. Create a new Docker image for launching the Bigbytes App— you can start with [our template](https://github.com/getbigbytes/docker/blob/master/Dockerfile). Upload this Docker image to a repository like Amazon ECR so ECS tasks can pull and create Docker containers using this image.
...
3. Create a new task definition based off the template below. This task, when
   ...
   1. Creates a new Bigbytes repository named "default_repo".
   2. Starts the Bigbytes app, serving requests sent to "localhost:6789"

   ```json
   {
     "containerDefinitions": [
       {
         "name": "bigbytes-data-prep-start",
         ...
         "essential": true,
         "entryPoint": ["sh", "-c"],
         "command": ["bigbytes init default_repo && bigbytes start default_repo"],
         "interactive": true,
         "pseudoTerminal": true
       }
     ],
     "family": "bigbytes-data-prep",
     "networkMode": "awsvpc",
...
4. Launch the task in your new cluster. Make sure the following conditions are
   ...
   - Task security group allows an inbound TCP connection to port 6789 from your
     ...
     to port 6789 where the Bigbytes app is running.
...
5. Find the public IP of your task. Then to access the Bigbytes app, go to
   `your-public-ip:6789`. If you followed the previous steps correctly, you
   should be able to access the Bigbytes app running in your cluster.

## Running Bigbytes Pipeline in ECS

If you already have a Bigbytes pipeline developed, you can deploy the pipeline as an
ECS task that periodically executes.

Follow the steps below to setup running a Bigbytes pipeline in ECS.

**Prerequisite:** You must have already created a Bigbytes repository containing the
