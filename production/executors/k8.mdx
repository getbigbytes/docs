...
<ProBanner
  ...
  title="Use Bigbytes Pro if you need to run your blocks in separate pods or process while developing in the code editor."
  description={`
...
If your Bigbytes app is running in a Kubernetes cluster, you can execute the blocks in separate Kubernetes pods with Kubernetes executor.
...
- uuid: example_data_loader
  ...
  ...
```
By default, Bigbytes uses `default` as the Kubernetes namespace. You can customize the namespace by setting the `KUBE_NAMESPACE` environment variable.
...
in this project. Example config:
    ...
    ```
* The kubernetes job name is in this format: `bigbytes-{job_name_prefix}-block-{block_run_id}`. The default `job_name_prefix` is `data-prep`. You can customize it in the k8s executor config.
...
* To further customize the container config of the kubernetes executor, you can specify the `container_config` or `job` in the k8s executor config. Here is the example:
  ...
  k8s_executor_config:
    container_config:
      image: getbigbytes/bigbytes:0.9.7
      env:
      - name: USER_CODE_PATH
...
**NB:** When deploying Bigbytes in a multi-container pod, you need to specify the environment variable `BIGBYTES_CONTAINER_NAME`. If this variable is not set, Bigbytes will default to using the first container in the pod. To specify the Bigbytes container, you can use:

```yaml
    env:
      - name: BIGBYTES_CONTAINER_NAME
        valueFrom:
          fieldRef:
