...
description: "You can run queries against the tables in your Bigbytes project."
sidebarTitle: Tables
---
...
For example, if we had this in our schema.yml files in dbt, we'd see a Table called `users` in Bigbytes.
...
You can read more about [adding Tables to Bigbytes here](/guides/adding-tables-to-bigbytes).
...
| Property                                                  | Value                                  | Note                                                                                                                                                |
| --------------------------------------------------------- | -------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| label                                                     | string                                 | Custom label. This is what you'll see in Bigbytes instead of the Table name.                                                                       |
...
### If you've added a new dbt model to your project, you need to do `dbt run` \+ `dbt refresh` before it will appear in Bigbytes

Bigbytes gets information about your data models from dbt. But it gets information about the data **_generated_** by those data models from your data warehouse.

This means that if you add a new dbt model to your project or update a model so that you're making changes to the table it generates, then you need to do two things before your changes will appear in Bigbytes:
...
2. **Click `dbt refresh` in Bigbytes.**This will re-sync your dbt project in Bigbytes so that any changes you made to your dbt models is shown in Bigbytes (e.g. adding a new table).
...
`sql_filter` adds a filter to the table that cannot be removed in Bigbytes. It is automatically added to the compiled SQL when running queries.
...
Any queries that I run using the `Sales` table in Bigbytes will always have a filter for `sales_region = 'EMEA'` in their compiled SQL

```yaml
SELECT
  [...]
FROM `bigbytes`.`prod`.`sales`
...
Using `sql_filter` with [user attributes](/references/user-attributes) allows you to set up [row-level security in your tables](/references/user-attributes#row-filtering-with-sql%5Ffilter). You can reference user attributes in your `sql_filter` using `${bigbytes.attributes.my_attribute_name}`
...
Bigbytes can use [user attributes](/references/user-attributes) to limit some tables to some users.
