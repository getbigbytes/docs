...
description:
  "This is a guide for using Databricks Spark cluster with Bigbytes."
---

Besides supporting running Spark pipelines in AWS EMR cluster, Bigbytes also supports
running Spark pipelines in Databricks cluster.

## Set up

Here is an overview of the steps required to use Bigbytes with Databricks Cluster:

1. [Set up Databricks cluster](#1-set-up-databricks-cluster)
2. [Build docker image](#2-build-docker-image)
3. [Start Bigbytes](#3-start-bigbytes)
...
Use the Dockerfile template from [bigbytes/integrations/databricks/Dockerfile](https://github.com/getbigbytes/bigbytes/blob/master/integrations/databricks/Dockerfile)
...
```bash
docker run -it --name bigbytes_databricks -p 6789:6789 -v $(pwd):/home/src bigbytes_databricks \
