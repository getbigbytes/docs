...
To introduce how the Transformer Actions library works, we will perform some cleaning actions on the Titanic Survival dataset. Open the Bigbytes app and create a new pipeline. Add a data loader with the following code to load the Titanic Survival dataset:
```python
...
![Image of transformer block template drop down highlighting "Fill in missing values"](https://github.com/getbigbytes/assets/blob/main/blocks/transformers/transformer_actions_impute.png?raw=true)
...
from bigbytes.data_cleaner.transformer_actions.constants import ActionType, Axis, ImputationStrategy
...
![Image of transformer block template drop down highlighting "Aggregate/Aggregate by average value"](https://github.com/getbigbytes/assets/blob/main/transformer_actions_agg.png?raw=True)
...
Source: `bigbytes.data_cleaner.transformer_actions.utils.build_transformer_action`
...
- **_outputs_:** Specifies the metadata for each output column of the aggregation (corresponding to the aggregation created from input argument in _arguments_). This array must be the same length as _arguments_, and each entry must contain the following metadata:
    ...
    - `column_type (string | ColumnType)`: Type of the new column ( See `bigbytes.data_cleaner.column_types.constants.ColumnType` for options)
...
These are list of column types used in the Bigbytes tool. One of these column types should be specified for every entry in the _options_ argument of a transformer action. The enum source is `bigbytes.data_cleaner.column_types.constants.ColumnType`.
