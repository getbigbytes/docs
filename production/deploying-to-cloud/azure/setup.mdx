...
Here is a high level diagram of how Bigbytes interacts with various Azure resources deployed using
this Terraform template.

![Bigbytes Azure architecture](https://github.com/getbigbytes/assets/blob/main/production/deployment/azure/architecture.png?raw=true)

## Pre-requisites

- Download Bigbytes maintained [Terraform templates](/production/deploying-to-cloud/using-terraform).
...
[./azure/variables.tf](https://github.com/getbigbytes/bigbytes-terraform-templates/blob/master/azure/variables.tf)
file.
...
In the file
[./azure/main.tf](https://github.com/getbigbytes/bigbytes-terraform-templates/blob/master/azure/main.tf),
...
1. Deploy
    ...
    id = "/subscriptions/d17fcd12-b89c-4f81-a221-40b8e768a0e8/resourceGroups/bigbytes-data-prep2-production/providers/Microsoft.ContainerInstance/containerGroups/bigbytes-data-prep-production"
    ip = "20.3.85.62"
    ```
...
```bash
az container restart --name bigbytes-data-prep-production --resource-group bigbytes-data-prep-production
```
...
In order to access Bigbytes on Azure from another local workstation, you must add
that IP address to the security group named
`bigbytes-data-prep-production-nsg-public`.
