...
You can create a personal access token (PAT) to authenticate in the [CLI](/get-started/setup-bigbytes/get-project-bigbytes-ready) or with the API.
...
```bash
curl --location --request GET 'https://my.getbigbytes.com/api/v1/org/projects' \
...
```bash
curl --location --request PATCH 'https://my.getbigbytes.com/api/v1/user/me/personal-access-tokens/<personal-access-token-uuid>/rotate' \
